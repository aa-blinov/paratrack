...
- **Per-activity colour coding restored** — `colorFor` hashes the lower-cased name onto a curated 10-colour palette (indigo, sky, teal, emerald, lime, amber, orange, violet, purple, pink), so each new activity lands on a stable, distinguishable slot. The 2px `.activity-mark` accent returns to lists, goals and the graph legend; chart series reuse the same colour, so chips and stacked bars share one cue. Pure red is reserved for destructive actions.
